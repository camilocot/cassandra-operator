...
func nodesForCassandra(c *v1alpha1.Cassandra) ([]string, error) {
	podList := podList()
	labelSelector := labels.SelectorFromSet(labelsForCassandra(c.Name)).String()
	listOps := &metav1.ListOptions{LabelSelector: labelSelector}
	err := sdk.List(c.Namespace, podList, sdk.WithListOptions(listOps))
	if err != nil {
		return nil, err
	}
